...
export default function App() {
  ...
  const [tab, setTab] = useState<'downloads' | 'tidal'>('downloads')
  ...
  return (
    <div className="min-h-full transition-colors duration-300">
      <header className="border-b border-slate-200 dark:border-gray-800 bg-white/80 dark:bg-gray-950/80 backdrop-blur transition-colors duration-300">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-slate-900 dark:text-gray-100">
              {tab === 'downloads' ? 'Descargas' : 'Tidal Search'}
            </h1>
            <p className="text-sm text-slate-500 dark:text-gray-400">
              {tab === 'downloads' ? 'Pegá la URL para iniciar una nueva descarga' : 'Busca canciones, álbumes o artistas en Tidal'}
            </p>
          </div>
          <ThemeToggle />
        </div>
        <div className="max-w-5xl mx-auto px-4 pb-2">
          <div className="inline-flex rounded-md border border-slate-200 dark:border-gray-800 overflow-hidden">
            <button
              onClick={() => setTab('downloads')}
              className={`px-3 py-1.5 text-sm transition-colors ${
                tab === 'downloads'
                  ? 'bg-slate-100 dark:bg-gray-800 text-slate-900 dark:text-gray-100'
                  : 'bg-white dark:bg-gray-950 text-slate-700 dark:text-gray-300 hover:bg-slate-50 dark:hover:bg-gray-900'
              }`}
            >
              Descargas
            </button>
            <button
              ...
              className={`px-3 py-1.5 text-sm border-l border-slate-200 dark:border-gray-800 transition-colors ${
                tab === 'tidal'
                  ? 'bg-slate-100 dark:bg-gray-800 text-slate-900 dark:text-gray-100'
                  : 'bg-white dark:bg-gray-950 text-slate-700 dark:text-gray-300 hover:bg-slate-50 dark:hover:bg-gray-900'
              }`}
            ...
            <div className="flex items-center justify-between px-4 pb-2 flex-wrap gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  title="Oculta las tareas completadas de la lista actual"
                >
                  Limpiar Completados
                </button>
                <button
